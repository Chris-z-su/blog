...
**进程**是执行程序的一次执行过程，是系统资源分配的基本单位。一个进程可以包含多个线程，多个线程可以共享数据。

**线程**是CPU调度和执行的基本单位。同类的多个线程共享进程的**堆**和**方法区**资源，但每个线程有自己的**程序计数器**、**虚拟机栈**和**本地方法栈**，所以系统在产生一个线程，或是在各个线程之间作切换工作时，负担要比进程小得多，也正因为如此，线程也被称为轻量级进程。
...
微服务是一种经过良好架构设计的分布式架构方案，微服务架构特征：单一职责、面向服务、自治和隔离性强。

单体架构：简单方便、耦合度高、可扩展性差

分布式架构：松耦合、扩展性好，但是架构复杂、难度大



微服务拆分粒度更小，服务与服务之间独立，耦合度低
