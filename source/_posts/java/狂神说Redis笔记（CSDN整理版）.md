---
title: 狂神说Redis笔记（CSDN整理版）
excerpt: 狂神说Redis笔记（CSDN整理版）
date: 2022-06-11 17:15:31
update: 2022-06-11 17:15:31
categories: 
  - Redis
tags: 
  - 数据库
...
4. 通过制定的配置文件启动redis服务
   ...
   [root@10 bin]# pwd
   /usr/local/bin
   [root@10 bin]# redis-server rdconf/redis.conf 
   [root@10 bin]# ps -ef|grep redis
   root       91425       1  0 10:04 ?        00:00:00 redis-server 127.0.0.1:6379
   root       99226    2639  0 10:07 pts/0    00:00:00 grep --color=auto redis
   [root@10 bin]# 
   ```
...
5. 使用redis-cli连接指定的端口号测试，Redis的默认端口6379
   ...
   ```
   [root@10 bin]# redis-cli -p 6379
   127.0.0.1:6379> 
   [root@10 ~]# ps -ef|grep redis
   root       91425       1  0 10:04 ?        00:00:00 redis-server 127.0.0.1:6379
   root      101011    2639  0 10:08 pts/0    00:00:00 redis-cli -p 6379
   root      105207  104194  0 10:09 pts/2    00:00:00 grep --color=auto redis
   [root@10 ~]# 
